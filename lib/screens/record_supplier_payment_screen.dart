...
/// Records a payment to a supplier against what's owed to them. A
/// supplier can be owed in either currency (or both), so this screen
/// picks which currency's balance is being paid down and validates
/// against that currency's own figure - never a blended total. The
/// amount also leaves Petty Cash as a real expense (see
/// services/supplier_payments.dart).
class RecordSupplierPaymentScreen extends StatefulWidget {
  const RecordSupplierPaymentScreen({
    super.key,
  ...
  final Map<AppCurrency, double> owedByCurrency;
...
class _RecordSupplierPaymentScreenState
  ...
  /// Defaults to whichever currency this supplier is actually owed in, so
  /// the common case (owing in only one currency) needs no extra tap.
  AppCurrency _initialCurrency() {
    for (final currency in AppCurrency.values) {
      if ((widget.owedByCurrency[currency] ?? 0) > 0.01) {
        return currency;
      }
    }
  ...
  double get _owed => widget.owedByCurrency[_selectedCurrency] ?? 0;
  ...
  Future<void> _save() async {
    ...
    if (amount > _owed + 0.01) {
      setState(
        () => _errorMessage =
            'You only owe ${widget.supplierName} '
            '${formatMoney(_owed, _selectedCurrency)}.',
      );
      return;
    }
